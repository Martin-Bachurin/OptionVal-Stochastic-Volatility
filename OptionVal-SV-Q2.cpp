...
int main () {
  ...
  // Discounting the call price to time 0, and calculating the antithetic call price
        ...
        // Compute the implied volitility with the current strike.
  ...
  // Compute the confidence interval with the current strike, and display the results.
        ...
        std::cout<<"Implied volitility when K = "<<K<<" "<<Isigma * 100<<"\n\n";
        std::cout<<"CI when K = "<<K<<" "<<Isigma_low * 100<<" "<<Isigma_high * 100<<"\n";
   }
   Exit();
}
